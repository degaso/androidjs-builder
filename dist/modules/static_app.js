...
class StaticApp extends webview_1.Webview {
    ...
    installModule(env, context) {
        ...
        //@ts-ignore
        env.sdk = this.sdk;
        (0, checkForUpdates_1.getUpdate)(env);
    ...
    ///  - update app name
    create() {
        let examplesFolder = (0, createIfNotExist_1.createIfNotExist)(path.join(this.env.builder.cache, '..', 'examples'));
        ...
        const exampleRepoDownloadLink = (0, GitListDir_1.getDownloadLink)(this.example.user, this.example.repo);
        ...
        if (!fs.existsSync(exampleFolderPath) || this.env.force) {
            ...
            downloadsdk({
                url: (0, GitListDir_1.getDownloadLink)(this.example.user, this.example.repo),
    ...
    downloadSDK(callback, force = false) {
        ...
        if (!force && fs.existsSync(sdkFolder)) {
            ...
            console.log((0, GitListDir_1.getDownloadLink)(this.sdk.user, this.sdk.repo));
            downloadsdk({
                url: (0, GitListDir_1.getDownloadLink)(this.sdk.user, this.sdk.repo, 'main'),
                repo: this.sdk.repo,
    ...
    updateFiles(callback) {
        ...
        // updating icon
        (0, updateIcon_1.updateIcon)(this.env, this);
        // updating app name
        (0, updateAppName_1.updateAppName)(this.env, this);
        // updating app version
        (0, updateApkVersion_1.updateApkVersion)(this.env, this);
        // generate Android Manifest file
        const manifestFileData = (0, ManifestBuilder_1.getManifest)(this.env, this, permissions, deep_links, screenOrientation);
    ...
    rebuildApk(callback) {
        ...
        const apkToolLink = (0, GitListDir_1.getFileDownloadLink)(this.apk_tool.user, this.apk_tool.repo, this.apk_tool.dir, this.apk_tool.file);
        const apkToolFilePath = path.join(this.env.builder.dir, 'build_tools', this.apk_tool.file);
        const apkSignerLink = (0, GitListDir_1.getFileDownloadLink)(this.apk_signer.user, this.apk_signer.repo, this.apk_signer.dir, this.apk_signer.file);
        ...
        const proc = (0, child_process_1.spawn)('java', args_, { cwd: cacheFolderPath });
        let log = "";
    ...
    build() {
        ...
        // check Java version
        (0, getJavaVersion_1.javaVersion)((error, home) => {
            if (error) {
                ...
                this.downloadSDK(() => {
                    ...
                    this.updateFiles((error) => {
                        (0, updateAppTheme_1.updateTheme)(this.env, (err) => {
                            this.rebuildApk((error) => {
                                this.sign(() => {
                                    try {
                                        ...
                                        (0, checkForUpdates_1.getUpdateMessage)();
                                    }
                                    catch (e) {
    ...
    sign(callback) {
        ...
        // @ts-ignore
        const proc = (0, child_process_1.spawn)('java', args_, { cwd: build_working_dir });
        proc.stdout.on('data', data => {
            if (progress.isRunning === false) {
